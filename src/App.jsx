...
export default function App() {
  ...
  function handleLogin(e) {
    ...
    // Basic check — just make sure fields are not empty
    if (!username.trim()) { triggerError("Please enter a username."); return; }
    if (!password.trim()) { triggerError("Please enter a password."); return; }

    // Simulate a short loading delay then log in — no real auth
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      setStep("success");
          ...
          <div className="fade-up" style={{animationDelay:"80ms",width:"100%",maxWidth:440}}>
            <div style={{
                ...
                {ROLES.map((r, i) => (
                  <button
                    ...
                    onMouseEnter={e => {
                      ...
                      e.currentTarget.style.boxShadow = `0 4px 16px ${r.color}18`;
                    }}
                    onMouseLeave={e => {
              ...
              <div style={{
                ...
                🧪 <strong>Testing mode</strong> — enter any username and password to log in
              </div>
            </div>
          </div>
        )}
            ...
            <div style={{
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      border:`1px solid ${error && !username ? "var(--error)" : "var(--border)"}`,
                ...
                <div style={{
                  ...
                  🧪 Testing mode — any username and password will work
                </div>
          ...
          <div className="fade-up" style={{width:"100%",maxWidth:420,textAlign:"center"}}>
            <div style={{
              ...
              <div style={{
                ...
                <span style={{fontSize:13,fontWeight:500,color:selectedRole.color}}>
                  Logged in as {selectedRole.label}
                </span>
              </div>
              ...
              {/* Placeholder dashboard hint */}
              <div style={{
                background:"var(--light)",borderRadius:12,border:"1px dashed var(--border)",
                padding:"16px",marginBottom:24,textAlign:"left",
              }}>
                <p style={{fontSize:12,color:"var(--muted)",marginBottom:8,fontWeight:500}}>NEXT STEP — Dashboard for {selectedRole.label}</p>
                {role === "doctor"  && <p style={{fontSize:13,color:"var(--text)"}}>📊 Your consultation scores and AI feedback</p>}
                {role === "owner"   && <p style={{fontSize:13,color:"var(--text)"}}>🏥 Clinic-wide performance overview</p>}
                {role === "admin"   && <p style={{fontSize:13,color:"var(--text)"}}>⚙ Full platform controls and leaderboard</p>}
              </div>
              ...
              <button
                ...
                style={{
                  ...
                  background:"var(--light)",fontSize:14,
